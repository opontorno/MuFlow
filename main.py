...
def parse_args():
    ...
    parser.add_argument('-l_contr', '--lambda_contrastive', type=float, default=1e-3, help="Weight for contrastive loss")
    parser.add_argument('-l_rec', '--lambda_reconstruction', type=float, default=1.0, help="Weight for reconstruction loss")
    parser.add_argument('-noise_mag', '--noise_magnitude', type=float, default=1e-5, help="Minimum noise multiplier (order of magnitude) relative to feature_std. The actual minimum noise std is noise_magnitude * feature_std.")
    parser.add_argument('-noise_max', '--noise_std_max', type=float, default=5.0, help="Maximum noise multiplier relative to feature_std. The actual maximum noise std is noise_std_max * feature_std. Must be >= noise_magnitude.")
    parser.add_argument('-margin', '--contrastive_margin', type=float, default=2.0, help="Margin for Siamese contrastive loss")
    parser.add_argument('-loss_type', '--contrastive_loss_type', type=str, default='contrastive', choices=['contrastive', 'infonce'], help="Type of contrastive loss: 'contrastive' (Siamese) or 'infonce' (InfoNCE)")
    parser.add_argument('-tau', '--infonce_temperature', type=float, default=0.07, help="Temperature for InfoNCE loss")
...
def train_projection_one_epoch(dataloader_pair, model, optimizer_proj, epoch, args):
    ...
    """
    from muflow.modules import contrastive_loss, infonce_loss
    ...
    noise_magnitude = args.noise_magnitude if hasattr(args, 'noise_magnitude') else 1.0
    noise_std_max = args.noise_std_max if hasattr(args, 'noise_std_max') else 5.0
    margin = args.contrastive_margin if hasattr(args, 'contrastive_margin') else 1.0
    loss_type = args.contrastive_loss_type if hasattr(args, 'contrastive_loss_type') else 'contrastive'
    temperature = args.infonce_temperature if hasattr(args, 'infonce_temperature') else 0.07
    ...
    for step, (img1, img2) in enumerate(dataloader_pair):
        ...
        with torch.no_grad():
            ...
            # Add noise to features1 for negative pairs (variable per-sample noise)
            noisy_features1 = []
            for feature in features1:
                B = feature.shape[0]
                feature_std = feature.std(dim=[0, 2, 3], keepdim=True) + 1e-8  # (1, C, 1, 1)
                # Sample a per-sample noise multiplier uniformly in [noise_magnitude, noise_std_max]
                scale = torch.empty(B, 1, 1, 1, device=feature.device).uniform_(noise_magnitude, noise_std_max)
                noise = torch.randn_like(feature) * scale * feature_std  # (B, C, H, W)
                noisy_features1.append(feature + noise.detach())
        ...
        for i in range(len(proj_clean1)):
            ...
            if loss_type == 'infonce':
                # InfoNCE loss
                # Anchors: proj_clean1, Positives: proj_clean2, Negatives: proj_noisy1
                loss_c = infonce_loss(
                    anchors=proj_clean1[i],
                    positives=proj_clean2[i],
                    negatives=proj_noisy1[i],
                    temperature=temperature,
                )
            else:
                # Siamese Contrastive loss
                # Positive pairs: (clean1, clean2) -> label=1
                # Negative pairs: (clean1, noisy1) -> label=0
                ...
                labels = torch.cat([
                    torch.ones(batch_size, device=device),   # positive
                    torch.zeros(batch_size, device=device)   # negative
                ], dim=0)
        ...
        # Total loss
        if epoch == 0 :
            loss_proj = lambda_r * loss_recon_avg
        else:
...
if __name__ == "__main__":
    ...
    if args.run_name is None:
        ...
        if args.use_proj_layer == 1:
            args.run_name += f"_proj-lc{args.lambda_contrastive}_lr{args.lambda_reconstruction}_noise{args.noise_magnitude}-{args.noise_std_max}_{args.contrastive_loss_type}"
